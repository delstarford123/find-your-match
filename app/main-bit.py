...
import random
import datetime
import threading  # CRITICAL: Added back for non-blocking CPU inference
...
from flask import Flask, render_template, session, redirect, url_for, flash, request, jsonify
# Ensure your database helpers are imported
from app.database import db, increment_restaurant_view
# 1. Path Setup & Environment
load_dotenv()
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))

# 2. Local App Imports (FIXED: Added get_all_restaurants & delete_user_account)
...
    get_all_restaurants, delete_user_account
)
...
app.config['VAPID_PRIVATE_KEY'] = "private_key.pem" # Path to your private key file
...
# ==========================================
# HELPER: REAL AI COMPANION LOGIC
# ==========================================
...
            vapid_claims={"sub": "mailto:delstarfordisaiah@gmail.com"}
        )
    except WebPushException as ex:
        print(f"Push failed: {ex}")

# Inside record_swipe() if is_match == True:
# push_data = json.dumps({
#    "title": "It's a Match! 🔥",
#    "body": f"You and {target_name} liked each other. Say hi!",
#    "url": "/matches"
# })
# send_push_notification(target_user_subscription, push_data)
import json
from pywebpush import webpush, WebPushException
...
    """
    # 1. Fetch the target student's device subscription from Firebase
...
    # 2. Craft the message they will see on their lock screen
    payload = json.dumps({
        "title": "It's a Match! 🔥",
...
    # 3. Fire the notification using your VAPID keys
    try:
        webpush(
            subscription_info=sub_ref,
            data=payload,
            vapid_private_key="private_key.pem", # Points to your new file
            vapid_claims={"sub": "mailto:delstarfordisaiah@gmail.com"}
        )
...
        # If the user revoked permission or changed phones, Google returns a 410 error.
        # We should delete the dead token to keep our database clean.
        if ex.response and ex.response.status_code == 410:
            db.reference(f'push_subscriptions/{target_user_id}').delete()
            print(f"🧹 Cleaned up expired push token for {target_user_id}")
            
# ==========================================
# 1. CORE B2C PAGES (STUDENTS)
# ==========================================
...
@app.route('/safety')
def safety():
    """Trust and Community Guidelines Page."""
    return render_template('safety.html', current_user=session.get('user_name'))

# Add send_from_directory to your flask imports at the top:
# from flask import Flask, render_template, session, redirect, url_for, flash, request, jsonify, send_from_directory
...
@app.route('/sw.js')
def service_worker():
    # Service workers must be served with a specific content type
...
@app.route('/venues')
def venues():
    """Public Directory of Partner Date Spots."""
...
# Assuming db is your initialized firebase_admin.db reference
# Assuming trigger_match_notification is already defined above this
...
    action = data.get('action') # Will be 'like' or 'pass'
    timestamp = datetime.now().isoformat()

    if not target_user_id or not action:
        return jsonify({"status": "error", "message": "Missing swipe data"}), 400

    # 2. Save the Swipe to Firebase
    # Structure: swipes -> current_user_id -> target_user_id -> {action, timestamp}
    try:
...
    # 3. Check for a Mutual Match (Only if they swiped 'like')
    if action == 'like':
        # Look into the target user's swipe history to see if they liked the current user
...
            # A. Create a permanent Match Record
            # We sort the IDs alphabetically so user1_user2 is the same string for both users
...
            # B. Fetch Profiles for the UI and Notification
...
            # Get just their first names
            current_name = current_profile.get('name', 'Someone').split(' ')[0]
            target_name = target_profile.get('name', 'Your Match').split(' ')[0]
            target_img = target_profile.get('img', '/static/img/placeholder.png')

            # C. Package data for the frontend 'IT'S A MATCH' overlay
            match_details = {
                'name': target_name,
                'img': target_img
            }

            # D. Fire the Background Push Notification to the target's phone!
            try:
                trigger_match_notification(target_user_id, current_name)
            except Exception as e:
                print(f"Failed to send push notification: {e}")

    # 4. Return the result to the JavaScript frontend
    return jsonify({
        "status": "success",
        "match": is_match,
...
            # Code is correct! Verify the user and delete the code from the database
            user_ref.update({
                'is_verified': True,
                'verification_code': None 
            })
            flash("Email verified successfully! Welcome to the club.", "success")
            return redirect(url_for('dashboard'))
        else:
            flash("Invalid verification code. Please try again.", "error")

    return render_template('verify.html', email=session.get('user_email'))
...
    try:
        # We store/overwrite the subscription for this specific user
        # This ensures that if they log in on a new phone, the old one is updated
...
def trigger_match_notification(user_id, partner_name):
    # 1. Fetch the stored subscription from Firebase
    sub_info = db.reference(f'push_subscriptions/{user_id}').get()
    
    if not sub_info:
        return # User hasn't enabled notifications yet

    # 2. Craft the Payload
    payload = json.dumps({
        "title": "It's a Match! 🔥",
        "body": f"You and {partner_name} liked each other. Say hi!",
        "url": "/matches"
    })

    # 3. Send via VAPID Handshake
    try:
        webpush(
...
    except WebPushException as ex:
        print(f"Push failed for {user_id}: {ex}")
            
...
# ==========================================
# 3. GOD MODE: SUPER ADMIN DASHBOARD
# ==========================================

@app.route('/admin/super', methods=['GET', 'POST'])
def super_admin():
    """The hidden founder dashboard."""
...
def admin_action():
    """Handles 1-click actions from the Admin Dashboard."""
    if not session.get('is_super_admin'):
...
    
    # 2. Calculate Student Revenue (20 KSH per paid user)
    paid_students = [p for p in profiles.values() if p.get('is_paid')]
    student_revenue = len(paid_students) * 20
    
    # 3. Calculate Merchant Revenue (2,000 KSH per active restaurant)
...
def process_swipe():
    data = request.json
    save_swipe(data['user_id'], data['target_id'], data['action'], datetime.datetime.now().isoformat())
...
        if result_code == 0:
            restaurant_id = "FETCHED_RESTAURANT_ID" 
            ref = db.reference(f'restaurants/{restaurant_id}')
            ref.update({
                'subscription_active': True,
                'subscription_expiry': (datetime.datetime.now() + datetime.timedelta(days=30)).isoformat()
            })
...
# ==========================================
# 5. WEBSOCKETS (CHAT, AI COMPANION & SAFETY)
# ==========================================
...
    # The data inside the QR code is a deep link to your app's verify endpoint
    # Example: https://match-ai.com/verify_customer/venue_golf_hotel
...
    
    # Convert image to base64 so we can show it in HTML without saving a file
    buf = io.BytesIO()
    img.save(buf)
...
    """This page is what the student sees after scanning the Merchant's QR."""
    # 1. Auth Check: Student must be logged in to their account
    if 'user_id' not in session:
...
    # 3. Safety Check: Does this restaurant exist?
    if not restaurant:
...
        color = "#10b981" # Green
        
        # 5. ALL ANALYTICS & ALERTS IN ONE BLOCK
        try:
            # A. Update permanent scan counter (Total Scans)
            qr_ref = db.reference(f'restaurants/{restaurant_id}/qr_scans')
            current_scans = qr_ref.get() or 0
            qr_ref.set(current_scans + 1)

            # B. Update Hourly Heatmap (For the 'Daily Summary' chart)
...
            # C. Push Real-time Merchant Notification (For the 'Cha-Ching' Alert)
